...
SELECT sal_emp,comision_emp,id_depto FROM empleados
WHERE id_depto LIKE '2000'
...
select nombre, id_depto,((sal_emp+comision_emp)+500) as 'pago estimado'
from empleados 
where id_depto = '3000'
order by nombre;
...
-- Obtener los nombres de los departamentos que sean “Ventas” ni “Investigación” ni ‘Mantenimiento --
SELECT cargo_emp FROM empleados
WHERE cargo_emp not in( 'ventas','investigacion','mantenimiento');
-- 20. Mostrar el salario más alto de la empresa. --
select nombre ,max(sal_emp) as 'salario mayor'
from empleados;
...
SELECT d.id_depto as "ID departamento", d.nombre_depto as "Nombre departamento", count(*) AS "Num empleados" 
...
SELECT nombre, cargo_emp, cod_jefe, count(*) as NumEmpleados FROM empleados Where cargo_emp in ('Jefe Ventas','Jefe Mercadeo','Jefe Mecanicos')
GROUP BY cod_jefe Having NumEmpleados > 2;

-- Hallar los departamentos que no tienen empleados--
SELECT d.id_depto, nombre_depto 
...
SELECT id_emp, nombre, sal_emp salario 
FROM empleados 
WHERE sal_emp > (SELECT AVG(sal_emp) FROM empleados) GROUP BY id_depto;